...
import {
  ...
  BookOpen, Users, BarChart2, Settings, LogOut, Headphones, Building2
} from 'lucide-react';
...
function Sidebar() {
  ...
  const navItem = (icon, label, path) => {
    const active = location.pathname === `/${slug}${path}`;
    return (
      <div
  ...
  const esAurogal = usuario?.empresa_id === 'emp-001';
    ...
    <div className="w-56 bg-[#1E3A5F] flex flex-col flex-shrink-0 min-h-screen">
        ...
        {esAurogal && (rol === 'admin' || rol === 'superadmin' || rol === 'tecnico') && (
          <>
            ...
            {(rol === 'admin' || rol === 'superadmin') &&
              navItem(<Users size={16} />, 'Usuarios', '/usuarios')}
            {rol === 'superadmin' &&
              navItem(<Building2 size={16} />, 'Empresas', '/empresas')}
            {navItem(<BarChart2 size={16} />, 'Reportes', '/reportes')}
            {(rol === 'admin' || rol === 'superadmin') &&
              navItem(<Settings size={16} />, 'Configuración', '/configuracion')}
          </>
        )}
      ...
      <div className="flex items-center gap-2 px-4 py-4 border-t border-white/10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[#6B8FAD] text-xs capitalize">{usuario?.rol}</p>
        </div>
        <button
